...
ModPE.langEdit("menu.copyright", "©Mojang AB                                                 Herobrine Mod 1.0.4b4");
...
function newLevel() {
    ...
    clientMessage("If you feel there is a new version, please type /download to download new version!")
}
...
function modTick() {
    timez--;
    alea = Math.floor(800 * Math.random() + 1);
    ...
    if (0 == spawned) {
        if (1 == alea && 0 == truetim && dspawn == false) {
        ...
        if (unstuck >= 1) {
            unstuck -= 1;
            if (0 == getTile(Entity.getX(Herobrine), Entity.getY(Herobrine), Entity.getZ(Herobrine)));
            else Entity.setPosition(Herobrine, Entity.getX(Herobrine), Entity.getY(Herobrine) + 1, Entity.getZ(Herobrine));
        }
    ...
    if (1 == spawned) {
        ...
        if (compareDistance(Herobrine, 15)) {
            Entity.remove(Herobrine);
            ModPE.playSoundFromFile("dissapear.ogg");
            preventOverridingTick = 400;
            spawned = 0;
        }
    }
    ...
    if (10 == wintimer)
        if (0 == spawned && 0 == truetim && dspawn == false) {
            pitch = (Entity.getPitch(getPlayerEnt()) + 90) * Math.PI / 180;
            yaw = (Entity.getYaw(getPlayerEnt()) + 90) * Math.PI / 180;
            orX = Math.sin(pitch) * Math.cos(yaw);
            orZ = Math.sin(pitch) * Math.sin(yaw);
...
function addFHerobrineRenderType(a) {
    ...
    var f = b.getPart("rightArm")
        .clear();
    var g = b.getPart("leftArm")
        .clear();
...
function entityRemovedHook(a) {
    ...
    for (var b in villager)
        if (a == villager[b]) {
            villager.splice(b, 1);
            return;
        }
    for (var b in mushroom)
        if (a == mushroom[b]) {
            mushroom.splice(b, 1);
            return;
        }
